...
namespace mrs_lib
{
   ...
   */
  static inline void UTM(double lat, double lon, float* x, float* y) {
    // constants
   ...
   *
   * Important: do not use this function output with UTMToLL() function, they are incompatible!
   *
   ...
   *
   * Important: initialize char* UTMZone[4] to avoid buffer overflow, use it with UTMtoLL() for reprojection.
   *
   * Written by Chuck Gantz- chuck.gantz@globalstar.com
   */
  static inline void LLtoUTM(const double Lat, const double Long, float& UTMNorthing, float& UTMEasting, char* UTMZone) {
    double a          = WGS84_A;
    ...
    UTMEasting =
        (float)(k0 * N * (A + (1 - T + C) * A * A * A / 6 + (5 - 18 * T + T * T + 72 * C - 58 * eccPrimeSquared) * A * A * A * A * A / 120) + 500000.0);

    UTMNorthing = (float)(k0 * (M + N * tan(LatRad) *
                                         (A * A / 2 + (5 - T + 9 * C + 4 * C * C) * A * A * A * A / 24 +
                                          (61 - 58 * T + T * T + 600 * C - 330 * eccPrimeSquared) * A * A * A * A * A * A / 720)));
    if (Lat < 0)
      UTMNorthing += float(10000000.0);  // 10000000 meter offset for southern hemisphere
  }

  static inline void LLtoUTM(const double Lat, const double Long, float& UTMNorthing, float& UTMEasting, std::string& UTMZone) {
   ...
   * Lat and Long are in fractional degrees.
   *
   * Important: obtain char*[4] UTMZone using the LLtoUTM() function.
   *
   * Written by Chuck Gantz- chuck.gantz@globalstar.com
   */
  static inline void UTMtoLL(const float UTMNorthing, const float UTMEasting, const char* UTMZone, double& Lat, double& Long) {
    double                  k0         = UTM_K0;
    double                  a          = WGS84_A;
    double                  eccSquared = UTM_E2;
    double                  eccPrimeSquared;
    ...
    x = double(UTMEasting) - 500000.0;  // remove 500,000 meter offset for longitude
  ...
  static inline void UTMtoLL(const float UTMNorthing, const float UTMEasting, std::string UTMZone, double& Lat, double& Long) {
